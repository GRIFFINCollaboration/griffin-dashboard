...
function regenerateDatastructure(suppressDOMconfig){
	...
	document.getElementById('triggerButton').onclick = function(){ RefreshDisplay(); };
	...
	for(i=0; i<FilterObjectID.length; i++){
	        ...
	        FilterReport.innerHTML = 'GRGa:<br>GRGb:<br>GRS:<br>'; 
		...
		for(i=0; i<9; i++){
	        ...
	        FilterReport.innerHTML = Namesstring; 
            }else{
		var Namesstring = '';
...
function repaint(){
	...
	if(FilterObjectID[i] == 'FilterBufferInput'){
	    document.getElementById(FilterObjectName).innerHTML = BuildFilterRatesValuesString(FilterObjectID[i],'Rate',1,dataStore.ODB.DAQ.params.DetTypes.length);
	}else if(FilterObjectID[i] == 'FilterObjectBGOSupp'){
	    document.getElementById(FilterObjectName).innerHTML = BuildFilterRatesValuesString(FilterObjectID[i],FilterSelectedDisplayType,1,4);
	}else if(FilterObjectID[i] == 'FilterObjectDetTypes' || FilterObjectID[i] == 'FilterObjectCoincDS'){
	    document.getElementById(FilterObjectName).innerHTML = BuildFilterRatesValuesString(FilterObjectID[i],FilterSelectedDisplayType,1,9);
            FilterObjectName = 'FilterObjectIDReportValuesB['+i+']';
	    document.getElementById(FilterObjectName).innerHTML = BuildFilterRatesValuesString(FilterObjectID[i],FilterSelectedDisplayType,9,dataStore.ODB.DAQ.params.DetTypes.length);
	}else{
            document.getElementById(FilterObjectName).innerHTML = BuildFilterRatesValuesString(FilterObjectID[i],FilterSelectedDisplayType,1,dataStore.ODB.DAQ.params.DetTypes.length);
	}
    }
...
// Unique report for the UDP output link to MIDAS
    ...
    document.getElementById('FilterTableTitleDiv').innerHTML = "UDP link to MIDAS from GRIF-C Master.";
}

function ReportInputLink(){
    // Reports for whichever Slave-Master input link is selected
    document.getElementById('FilterTableReportTitles').innerHTML = '';
    document.getElementById('FilterTableReportValuesPerc').innerHTML = '';
    document.getElementById('FilterTableReportValuesEvts').innerHTML = '';
    FilterSelectedElementID;
    var ColNum = FilterSelectedElementID.replace( /^\D+/g, '');
    document.getElementById('FilterTableTitleDiv').innerHTML = "Input link from GRIF-C Collector"+ColNum+" to GRIF-C Master.";
}
